...
const CustomTick = ({ payload, x, y, textAnchor }: any) => (
  <g transform={`translate(${x},${y})`}>
    <image href={categoryImageMap[payload.value] || "/default-image.png"} x={-15} y={-20} width={40} height={24} />
    <text textAnchor={textAnchor} fontSize="12px" fill="#333" y={10} dy="0.5em" className="font-bold">
      {payload.value}
    </text>
  </g>
);
  ...
  const generateDummyData = () => {
    ...
    while (day <= endMonth) {
      data[format(day, "yyyy-MM-dd")] = {
        url: categoryImageMap[categories[Math.floor(Math.random() * categories.length)]] || "/default-image.png",
    ...
    <div className="h-[900px] flex flex-col">
      <div className="flex flex-col p-4 bg-gray-100">
        <div className="flex justify-between mb-4">
          ...
          <button onClick={nextMonth} className="px-4 py-2 bg-yellow-400 text-black font-bold rounded-full flex">
        ...
        <div className="grid grid-cols-7 gap-1 text-center text-sm h-full">
          ...
          {generateCalendarDays().map((day, index) => {
            ...
            return (
              <div key={index} className={`border p-2 relative flex flex-col items-center justify-center ${isCurrentMonth ? "text-black" : "text-gray-400"}`} style={{ width: '50px', height: '50px' }}>
                <span className="absolute top-0 left-0 text-[7px]">{format(day, "d")}</span>
                {imageInfo.url && <Image src={imageInfo.url} alt="category image" width={30} height={30} className="my-1 cursor-pointer" onClick={()=>router.push('/')}/>}
                {hydrated && imageInfo.count > 0 && (
                  <span className="absolute bottom-0 right-0 text-sm text-gray-500">{imageInfo.count}</span>
                )}
              </div>
            );
          })}
      ...
      <div className="mt-5 flex flex-col items-center justify-center">
        ...
        <a className=" ml-3 text-lg font-medium" href="/category">できたんカテゴリーについて ＞</a>
        </div>
      </div>
    </div>
  );
